...
AVMI_diag_uni_cat_Rand{1}=nan(NbCell_estimate,length(StimTypeCM));
AVMI_diag_uni_cat_Rand{2}=AVMI_diag_uni_cat_Rand{1};
SDMI_diag_uni_cat_Rand=AVMI_diag_uni_cat_Rand;
MI_uni_diag_cat_Rand = cell(NbCell_estimate, 1000);
MI_uni_diag_cat_RandBG = cell(NbCell_estimate, 1000);


PCC_cat = cell(length(StimTypeCM),1);
for iss=1:length(PCC_cat);
    PCC_cat{iss} = cell(NbCell_estimate,1);
end
IS3=PCC_cat;
IS2 = zeros(NbCell_estimate,length(StimTypeCM));% the four column correspond to the four categorisatios (names, age, sex, ageXsex)
II2 = zeros(NbCell_estimate,length(StimTypeCM));% the four column correspond to the four categorisatios (names, age, sex, ageXsex)
...
H_invariance = cell(length(StimTypeCM),1);
H_invariance{1}=nan(NbCell_estimate,11);%there must be max 15 different names per confusion matrix
H_invariance{2}=nan(NbCell_estimate,length(StimTypeCM{2}));%2 ages C and A + B for background stims
H_invariance{3}=nan(NbCell_estimate,length(StimTypeCM{3}));%2 sexes F and M + B for background stims
H_invariance{4}=nan(NbCell_estimate,length(StimTypeCM{4}));%2 ages C and A x 2 sexes (F and M) + B for background stims
...
NVocPerCat = cell(length(StimTypeCM),1);
VocCatNames = NVocPerCat;
for cl=1:length(StimTypeCM)
    NVocPerCat{cl}=nan(NbCell_estimate,length(StimTypeCM{cl}));
...
SpikeRate_Cat = cell(length(StimTypeCM),1);
for cl=1:length(StimTypeCM)
    SpikeRate_Cat{cl}=nan(NbCell_estimate,length(StimTypeCM{cl})-1); %We don't need the spike rate during background that's why: StimTypeCM-1
end
...
for ss=1:length(Subjects)
 ...
 %% Upload the random matrices and calculate MI per area
            ...
            rt = 2;%I set it to 2 on 14/08/11 because we are not using values of random matrices without BG fixed
            while rt<=2
                if rt==1
                ...
                fprintf('the MI_confusion of the IV Matrix is %f for that cell\nThe average MI of random matrices is %f+/-%f\n', MI_tot(ii), AvMI_tot_Rand(ii,rt),SDMI_tot_Rand(ii,rt))
            %pause
                AVMI_diag_uni_cat_Rand{rt}(ii,:)=mean(MI_diag_uni_cat_rand,1);
                SDMI_diag_uni_cat_Rand{rt}(ii,:)=std(MI_diag_uni_cat_rand,1);
                if rt==1
